	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
             ...
             fixed4 colorOut = (1.0, 1.0, 1.0, 1.0 );
...
//        	 if(_ShowTexture<0.5) {
//
//        	     if (mipLevelFloor == 0.0 ) colorOut = tex2D(_DirtHigh, i.tex), weight;
// 				 else if (mipLevelFloor == 1.0 ) colorOut = tex2D(_DirtMed, i.tex), weight;
// 				 else if (mipLevelFloor == 2.0 ) colorOut = tex2D(_DirtLow, i.tex), weight;
//             }
//
              else
             {
               colorOut = tex2D(_MainTex, i.tex.xy*_MainTex_ST.xy + _MainTex_ST.zw, dx, dy);
